...
let game_over = new Promise((resolve, reject) => {
  ...
  game_start.then(user_and_diff => {
    ...
    const generate_enemy = function() {
      ...
      if (I % 3 === 0) Enemies.push(new Enemy(wrapper));
      else if (I % 3 === 1) Enemies.push(new EnemyCone(wrapper));
      else Enemies.push(new EnemyDiamond(wrapper));
    ...
    //generating enemies
    Rx.Observable.interval(ENEMY_GEN_SPEED).subscribe(generate_enemy, err => {
      console.log(err);
    });
    ...
    Rx.Observable.interval(FREQ_COLLISION_LISTENER).subscribe(
      collision_listener,
      err => {
        console.log(err);
      }
    ...
    Rx.Observable.interval(FREQ_FIELD_EMPTY_CHECK).subscribe(
      field_clear_checker,
      err => {
        console.log(err);
      }
    ...
    const bullets_hit_enemies = function() {
      player.bullets.forEach(function(bullet) {
        ...
        Enemies.forEach(function(enemy) {
          ...
          let x_match =
            Math.abs(bullet_rect.x - (enemy_rect.x + enemy_rect.width / 2)) -
              10 <=
            enemy_rect.width / 2;
          let y_match =
            Math.abs(enemy_rect.y - bullet_rect.y) <= enemy_rect.height / 2;
    ...
    Rx.Observable.interval(FREQ_BULLET_HITS).subscribe(
      bullets_hit_enemies,
      err => {
        console.log(err);
      }
